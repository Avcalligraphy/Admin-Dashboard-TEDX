...
function TransactionsEvent() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${apiUrl}/transaction/event`, {
        headers: {
        ...
        <SoftBox mb={3}>
          <Card>
            <SoftBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
              <SoftTypography variant="h6">Transactions Events Success</SoftTypography>
            </SoftBox>
            <SoftBox
              sx={{
...
export default TransactionsEvent;
